/**
 * Quick Chat — content script injected by Ctrl+K.
...
(async function QuickChat() {
  ...
  function hide() {
    ...
    container.remove();
    disconnect();
    // Save session for side panel continuity
    if (state.sessionId) {
      new SessionManager().load().then(() => {
  ...
  async function connect() {
    ...
    state.ws.on('message', (data) => {
      appendMsg('assistant', data.text || '', true);
  ...
  function send() {
    ...
    if (!text || !state.ws?.connected || state.isStreaming) return;
